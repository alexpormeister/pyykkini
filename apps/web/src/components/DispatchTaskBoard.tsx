...
import { fetchDispatchData, fetchLaundryPrices, TaskRow, DriverInfo, LaundryInfo, ProductInfo, LaundryProductPrice, BookingProductItem } from "@pesuni/shared";
...
export const DispatchTaskBoard: React.FC = () => {
  ...
  // 🔍 Puhelinnumero-ehdotukset & Autokompletointi (Viimeisen 1v tilaukset & profiilit)
  ...
  const { data: dispatchData, isLoading, refetch } = useQuery({
    queryKey: ['dispatchData'],
    queryFn: () => fetchDispatchData(supabase)
  });

  const tasks = dispatchData?.tasks || [];
  const drivers = dispatchData?.drivers || [];
  const laundries = dispatchData?.laundries || [];
  const products = dispatchData?.products || [];
  const dbTimeSlots = dispatchData?.dbTimeSlots || [];
  
  useEffect(() => {
    if (dispatchData?.settings) {
      if (dispatchData.settings.min_order_amount != null) setFormMinThreshold(String(dispatchData.settings.min_order_amount));
      if (dispatchData.settings.service_fee != null) setFormServiceFee(String(dispatchData.settings.service_fee));
      if (dispatchData.settings.delivery_fee != null) setFormDeliveryFee(String(dispatchData.settings.delivery_fee));
    }
  ...
  const { data: laundryPricesData } = useQuery({
    ...
    queryFn: () => fetchLaundryPrices(supabase, formLaundryId),
    enabled: !!formLaundryId
  });
  
  const laundryPrices = laundryPricesData || [];
  ...
  // 🔍 Suljetaan puhelinnumero-ehdotukset kun klikataan muualle
  useEffect(() => {
  ...
  // 🔍 Puhelinnumeron perusteella haku: Viimeisen 1v tilaukset & profiilit
  useEffect(() => {
    const searchCustomerHistory = async () => {
      const cleanPhone = formPhone.replace(/[^\d+]/g, "").trim();
      ...
      try {
        ...
        // 1. Haetaan viimeisen 1 vuoden tilaukset täsmäävällä puhelinnumerolla
        const { data: ordersData } = await supabase
          .from("orders")
          .select("id, user_id, first_name, last_name, phone, address, access_code, created_at")
          .gte("created_at", oneYearAgo.toISOString())
          .ilike("phone", `%${cleanPhone}%`)
          .order("created_at", { ascending: false })
          .limit(10);

        // 2. Haetaan asiakasprofiilit täsmäävällä puhelinnumerolla
        ...
        // Kanta-asiakasprofiilit
        if (profilesData) {
          for (const p of profilesData) {
            const key = `${p.phone || ""}_${p.first_name || ""}_${p.last_name || ""}_${p.address || ""}`.toLowerCase();
            if (!seenKeys.has(key)) {
              seenKeys.add(key);
              items.push({
                id: `profile_${p.user_id}`,
                userId: p.user_id,
                firstName: p.first_name || "",
                lastName: p.last_name || "",
                phone: p.phone || formPhone,
                address: p.address || "",
                source: 'profile',
              });
            }
        ...
        // Viimeisen 1v tilaushistorian tiedot
        if (ordersData) {
          for (const o of ordersData) {
            const key = `${o.phone || ""}_${o.first_name || ""}_${o.last_name || ""}_${o.address || ""}`.toLowerCase();
            if (!seenKeys.has(key)) {
              seenKeys.add(key);
              items.push({
                id: `order_${o.id}`,
        ...
        setPhoneSuggestions(items);
        setShowPhoneDropdown(items.length > 0);

        const matchedProfile = items.find(i => i.userId);
        setMatchedUserId(matchedProfile?.userId || null);
      } catch (err) {
    ...
    const timer = setTimeout(searchCustomerHistory, 300);
    return () => clearTimeout(timer);
  }, [formPhone]);

  // 📝 Valitse profiili/tilaus ehdotusvalikosta (Täyttää VAIN asiakas- & noutotiedot!)
  const handleSelectCustomerSuggestion = (sug: typeof phoneSuggestions[0]) => {
    if (sug.firstName) setFormFirstName(sug.firstName);
    if (sug.lastName) setFormLastName(sug.lastName);
    if (sug.phone) setFormPhone(sug.phone);
    if (sug.email) setFormEmail(sug.email);
    if (sug.accessCode) setFormAccessCode(sug.accessCode);
    if (sug.userId) setMatchedUserId(sug.userId);

    if (sug.address) {
      const parsed = parseStructuredAddress(sug.address);
      const fullStreet = [parsed.streetName, parsed.houseNumber, parsed.apartmentNumber].filter(Boolean).join(" ");
      setFormStreet(fullStreet || parsed.fullAddress || sug.address);
      if (parsed.postalCode) setFormPostalCode(parsed.postalCode);
      if (parsed.city) setFormCity(parsed.city);
    }

    setShowPhoneDropdown(false);

    toast({
      title: "Asiakastiedot täytetty! ✨",
      description: `Täytetty osoite- ja nimitiedot asiakkaalle ${sug.firstName} ${sug.lastName}`.trim(),
    });
  };
            ...
            <div className="p-3 rounded-lg border bg-muted/20 space-y-2.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                <div className="relative" ref={phoneContainerRef}>
                  <Label className="text-xs font-medium text-foreground/80 mb-1 flex items-center justify-between">
                    <span>Puhelin *</span>
                    {phoneSearchLoading && <span className="text-[10px] text-muted-foreground animate-pulse font-normal">Etsitään...</span>}
                  </Label>
                  <Input
                    value={formPhone}
                    onChange={(e) => {
                  ...
                  {/* 🔍 AUTOKOMPLETOINTI / VIIMEISEN VUODEN TILAUKSET & PROFIILIT DROPDOWN */}
                  {showPhoneDropdown && phoneSuggestions.length > 0 && (
                    <div className="absolute top-full left-0 right-0 sm:w-[320px] mt-1 z-50 bg-popover text-popover-foreground rounded-xl border shadow-xl overflow-hidden max-h-64 overflow-y-auto">
                      <div className="p-2 bg-muted/40 border-b flex items-center justify-between text-[11px] font-semibold text-muted-foreground px-3">
                        <span>Viimeisen 1v tilaukset & profiilit ({phoneSuggestions.length})</span>
                        <button
                          type="button"
                      ...
                      <div className="divide-y divide-border/40">
                        {phoneSuggestions.map((sug) => (
                          <button
                            key={sug.id}
                            type="button"
                            onClick={() => handleSelectCustomerSuggestion(sug)}
                            className="w-full text-left p-2.5 hover:bg-accent/60 transition-colors flex items-start gap-2.5 group"
                          >
                            <div className="mt-0.5 p-1.5 rounded-lg bg-primary/10 text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-colors shrink-0">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-1 mb-0.5">
                                ...
                                <span className="text-[10px] font-medium text-muted-foreground bg-muted px-1.5 py-0.2 rounded shrink-0">
                                  {sug.phone}
                                </span>
                              </div>
                              <div className="text-[11px] text-muted-foreground truncate flex items-center gap-1">
                                <MapPin className="h-3 w-3 shrink-0 opacity-70" />
                                <span className="truncate">{sug.address || "Ei osoitetta"}</span>
                                {sug.accessCode && (
                                  <span className="text-[10px] font-mono text-primary bg-primary/10 px-1 rounded ml-1 shrink-0">
                                    Koodi: {sug.accessCode}
                                  </span>
                                )}
                              </div>
                              {sug.lastOrderDate && (
                                <div className="text-[10px] text-emerald-600 dark:text-emerald-400 font-medium mt-0.5">
                                  Viimeisin tilaus: {formatSafeDate(sug.lastOrderDate)}
                                </div>
                              )}
                            </div>
                          </button>
