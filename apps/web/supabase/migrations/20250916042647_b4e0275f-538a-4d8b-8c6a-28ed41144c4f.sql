-- Fix get_driver_orders function to properly show accepted orders
...
BEGIN
  ...
  -- For drivers: always return their assigned orders regardless of shift status
  -- For pending orders, only show if on active shift  
  RETURN QUERY
  SELECT 
    o.id,
    o.pickup_date,
  ...
  FROM public.orders o
  WHERE 
    -- Always show assigned orders
    o.driver_id = auth.uid() 
    OR 
    -- Show pending orders only if driver is on shift
