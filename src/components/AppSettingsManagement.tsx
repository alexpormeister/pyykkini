...
export const AppSettingsManagement = () => {
  ...
  const handleSave = async () => {
    if ([form.service_fee, form.vat_rate, form.delivery_fee, form.min_order_amount].some((v) => isNaN(v) || v < 0)) {
  ...
  const total = exampleBase + form.service_fee + form.delivery_fee;
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-6 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">Lisätään automaattisesti jokaisen tilauksen loppusummaan.</p>
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="delivery_fee" className="flex items-center gap-2">
                <Truck className="h-4 w-4 text-muted-foreground" /> Oletustoimitusmaksu (€)
              </Label>
              <Input id="delivery_fee" type="number" step="0.01" min="0" value={form.delivery_fee} onChange={num("delivery_fee")} />
              <p className="text-xs text-muted-foreground">Käytetään, jos palvelualueelle ei ole omaa toimitusmaksua.</p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">0 = ei minimisummaa.</p>
            </div>
          </div>
          ...
          <div className="rounded-lg border bg-muted/40 p-4 space-y-1 text-sm">
            ...
            <div className="flex justify-between"><span className="text-muted-foreground">Toimitusmaksu</span><span>{form.delivery_fee.toFixed(2)} €</span></div>
